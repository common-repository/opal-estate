...
class Opalestate_Field_Adduser {
	...
	//	echo '<pre>'.print_r( $value, 1 );die;
		echo '<div class="opalestate-add-user-field '.apply_filters('opalestate_row_container_class', 'row opal-row').'"> '; ?>
			<div class="col-lg-12">
				<h5 class=""><?php _e( 'As an author, you can add other users to your office.', 'opalestate'); ?></h5>
				<div>
					<p><?php _e( 'Add someone to your office, please enter extractly username in below input:', 'opalestate' ); ?></p>
					...
					<div class="<?php echo apply_filters('opalestate_row_container_class', 'row opal-row'); ?>">
						...
						<div class="col-lg-4"><input name="search" class="button button-primary button-large pull-left" id="publish" value="<?php _e( 'add', 'opalestate' ); ?>" type="button"></div>
					</div>
				...
				<div class="adduser-team">
					<?php if( $users ): $users = array_unique( $users ); ?>
						...
						<?php foreach( $users as $user_id ): $user 		 = get_user_by( 'id', $user_id ); $user = $user->data ?>
							...
							<div> <span class="remove-user" data-alert="<?php _e( 'Are you sure to delete this', 'opalestate' ); ?>"><?php _e(  'Remove', 'opalestate' ); ?></span></div>
							</div>
						<?php endforeach; ?>
