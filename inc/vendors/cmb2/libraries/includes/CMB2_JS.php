...
class CMB2_JS {
	...
	protected static function colorpicker_frontend() {
		...
		wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
			'clear'         => esc_html__( 'Clear', 'opalestate' ),
			'defaultString' => esc_html__( 'Default', 'opalestate' ),
			'pick'          => esc_html__( 'Select Color', 'opalestate' ),
			'current'       => esc_html__( 'Current Color', 'opalestate' ),
		) );
	}
		...
		$l10n = array(
			...
			'defaults'          => array(
				...
				'date_picker'  => array(
					'changeMonth'     => true,
					'changeYear'      => true,
					'dateFormat'      => _x( 'mm/dd/yy', 'Valid formatDate string for jquery-ui datepicker', 'opalestate' ),
					'dayNames'        => explode( ',', esc_html__( 'Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday', 'opalestate' ) ),
					'dayNamesMin'     => explode( ',', esc_html__( 'Su, Mo, Tu, We, Th, Fr, Sa', 'opalestate' ) ),
					'dayNamesShort'   => explode( ',', esc_html__( 'Sun, Mon, Tue, Wed, Thu, Fri, Sat', 'opalestate' ) ),
					'monthNames'      => explode( ',', esc_html__( 'January, February, March, April, May, June, July, August, September, October, November, December', 'opalestate' ) ),
					'monthNamesShort' => explode( ',', esc_html__( 'Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec', 'opalestate' ) ),
					'nextText'        => esc_html__( 'Next', 'opalestate' ),
					'prevText'        => esc_html__( 'Prev', 'opalestate' ),
					'currentText'     => esc_html__( 'Today', 'opalestate' ),
					'closeText'       => esc_html__( 'Done', 'opalestate' ),
					'clearText'       => esc_html__( 'Clear', 'opalestate' ),
				),
				'time_picker'  => array(
					'timeOnlyTitle' => esc_html__( 'Choose Time', 'opalestate' ),
					'timeText'      => esc_html__( 'Time', 'opalestate' ),
					'hourText'      => esc_html__( 'Hour', 'opalestate' ),
					'minuteText'    => esc_html__( 'Minute', 'opalestate' ),
					'secondText'    => esc_html__( 'Second', 'opalestate' ),
					'currentText'   => esc_html__( 'Now', 'opalestate' ),
					'closeText'     => esc_html__( 'Done', 'opalestate' ),
					'timeFormat'    => _x( 'hh:mm TT', 'Valid formatting string, as per http://trentrichardson.com/examples/timepicker/', 'opalestate' ),
					'controlType'   => 'select',
					'stepMinute'    => 5,
				),
			),
			'strings' => array(
				'upload_file'  => esc_html__( 'Use this file', 'opalestate' ),
				'upload_files' => esc_html__( 'Use these files', 'opalestate' ),
				'remove_image' => esc_html__( 'Remove Image', 'opalestate' ),
				'remove_file'  => esc_html__( 'Remove', 'opalestate' ),
				'file'         => esc_html__( 'File:', 'opalestate' ),
				'download'     => esc_html__( 'Download', 'opalestate' ),
				'check_toggle' => esc_html__( 'Select / Deselect All', 'opalestate' ),
			),
		);
